...
abstract class DataTypeWriteCompatibilityBaseSuite extends SparkFunSuite {
    ...
    atomicTypes.foreach { w =>
      atomicTypes.foreach { r =>
        if (canCast(w, r)) {
          ...
          checkError(
            exception = intercept[AnalysisException] (
              DataTypeUtils.verifyCanWrite("", w, r, byName = true, analysis.caseSensitiveResolution, "t",
  ...
  def assertAllowed(
    ...
    DataTypeUtils.verifyCanWrite("", writeType, readType, byName, analysis.caseSensitiveResolution,
