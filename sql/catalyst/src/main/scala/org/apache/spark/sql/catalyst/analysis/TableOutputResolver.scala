...
object TableOutputResolver extends SQLConfHelper with Logging {
  ...
  def resolveField(
    ...
    (value.dataType, col.dataType) match {
      ...
      case (valueType, colType) if DataType.equalsIgnoreCompatibleNullability(valueType, colType) =>
        ...
        val withNulls = checkNullability(value, col, conf, colPath)
        // todo: do we need the col metadata?
        makeNamed(withNulls, col.name)
  ...
  private def resolveBasicType(
    ...
    expr = checkNullability(expr, attr, conf, colPath)
  ...
  private def makeNamed(expr: Expression, name: String): NamedExpression = {
    expr match {
      case expr: NamedExpression if name == expr.name => expr
      // Save an Alias if we can change the name directly.
      case a: Attribute => a.withName(name)
      case a: Alias => a.withName(name)
      case _ => Alias(expr, name)()
    }
  }

  //
  private def makeNamed(expr: Expression, attr: Attribute): NamedExpression = {
     makeNamed(expr, attr.name)
    //    // todo: do we need the attr metadata? if no, remove this method.
//        expr match {
...
//        }
  ...
  private def verifyCanWrite(
    ...
    conf.storeAssignmentPolicy match {
      ...
      // todo: Is this method used by v1?  Should it verify LEGACY writes? or block them?
    }
  }
  ...
  private def reorderColumnsByName(
    ...
    val reordered = expectedCols.map { expectedCol =>
      ...
      if (matched.isEmpty) {
        ...
        val matchedCol = makeNamed(matched.head, expectedName)
    ...
    if (!expr.dataType.existsRecursively(_.isInstanceOf[UserDefinedType[_]])) {
      ...
      expr.dataType match {
        ...
        case st: StructType =>
          ...
          if (expr.nullable) {
            // todo: this doubles expr evaluation.
